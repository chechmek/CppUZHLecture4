...
bool between(Iter fst, Iter lst, Iter i, input_iterator_tag) {
...
template<typename Iter>
bool between(Iter fst, Iter lst, Iter i, random_access_iterator_tag) {
	if (fst <= i && i <= lst)
		return true;
	return false;
}
...
int main()
{
	...
	cout << between<InputIterator>(itBeg, itEnd, it) << endl; 
}